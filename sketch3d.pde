...
boolean initializeKinect() {
  kinect = new SimpleOpenNI(this);
  mc.updateStatus( "Looking for Kinect..." );
  ...
  if (SimpleOpenNI.deviceCount() > 0 ) {
    ...
    mc.updateStatus( "Kinect found. Waiting for user..." );
...
void onNewUser(int userId) {
  mc.updateStatus( "User " + userId + " found.  Please assume Psi pose." );
  kinect.startPoseDetection("Psi", userId);
}

void onStartPose(String pose, int userId) {
  mc.updateStatus( pose + " pose detected for user " + userId + ". Requesting calibration skeleton." );
...
void onEndCalibration(int userId, boolean successful) {
  if (successful) { 
    mc.updateStatus( "Calibration ended successfully for user " + userId + " Tracking user." );
    kinect.startTrackingSkeleton(userId);
    skeleton.setUser(userId);
  } 
  else { 
    mc.updateStatus( "Calibration failed starting pose detection." );
    kinect.startPoseDetection("Psi", userId);
  }
}
...
void onLostUser(int userId) {
  mc.updateStatus( "User " + userId + " lost." );
}
